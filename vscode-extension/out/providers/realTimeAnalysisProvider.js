...
exports.RealTimeAnalysisProvider = void 0;
const vscode = __importStar(require("vscode"));
const path = __importStar(require("path"));
class RealTimeAnalysisProvider {
    constructor(connection) {
        this.connection = connection;
        this.analysisQueue = new Map();
        this.currentAnalyses = new Set();
    ...
    async analyzeDocument(document, isOnSave) {
        if (!this.isSupportedFile(document)) {
            return;
        }
        const filePath = document.uri.fsPath;
    ...
    async performAnalysis(document, isOnSave) {
        try {
            const filePath = document.uri.fsPath;
    ...
    updateDiagnostics(uri, analysis) {
        const diagnostics = [];
        // Convert analysis issues to VS Code diagnostics
        for (const issue of analysis.issues) {
            const range = new vscode.Range(Math.max(0, issue.line - 1), // VS Code is 0-indexed
            0, Math.max(0, issue.line - 1), 1000 // End of line
            );
    ...
    async analyzeWorkspace() {
        ...
        const files = await vscode.workspace.findFiles('**/*.{py,js,ts,jsx,tsx,go,rs,java,cpp,c,cs,php,rb}', '**/node_modules/**');
...
exports.RealTimeAnalysisProvider = RealTimeAnalysisProvider;
//# sourceMappingURL=realTimeAnalysisProvider.js.map