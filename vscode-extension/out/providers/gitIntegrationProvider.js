...
exports.GitIntegrationProvider = void 0;
const vscode = __importStar(require("vscode"));
class GitIntegrationProvider {
    ...
    async generateCommitMessage() {
        try {
            ...
            if (finalMessage) {
                ...
                const action = await vscode.window.showInformationMessage('Commit message generated and copied to clipboard!', 'Commit Now', 'Open Git Panel');
...
${changes.files_changed.map(f => `• ${f}`).join('\n')}
            `;
            const action = await vscode.window.showInformationMessage('Smart Commit Preview', {
                modal: true,
    ...
    async suggestBranchName() {
        try {
            ...
            if (response.success && response.branch_name) {
                ...
                const action = await vscode.window.showInformationMessage(`Suggested branch name: ${branchName}`, 'Create Branch', 'Copy Name', 'Edit Name');
    ...
    async showGitPanel() {
        const panel = vscode.window.createWebviewPanel('auraGit', 'Aura Git Assistant', vscode.ViewColumn.Two, {
            enableScripts: true,
...
exports.GitIntegrationProvider = GitIntegrationProvider;
//# sourceMappingURL=gitIntegrationProvider.js.map